...
const waitForLogMessageContaining = async (nrdb, substring) => {
    return nrdb.waitToFindOne({ where: `message like '%${substring}%' and plugin.type = 'azure'` });
}
...
const countAll = async (nrdb, substring, expectedCount) => {
    ...
    while(nRetries--) {
        ...
        else console.log('Count not matching, expected: ' + expectedCount + ', actual: ' + result.count)
