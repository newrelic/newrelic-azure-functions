...
| Authentication Mode | no | `Local Authentication` | Authentication method for the Event Hub connection. Use `Managed Identity` when local authentication (shared access keys) is disabled in your Azure environment for security compliance. When set to `Managed Identity`, the Function App is assigned a system-assigned managed identity and granted the `Azure Event Hubs Data Receiver` role on the Event Hub namespace — no connection string is stored. Use `Local Authentication` (default) to connect using a shared access key connection string. |
